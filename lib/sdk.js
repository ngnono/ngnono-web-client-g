...
Sdk.prototype._handler = function (name, route, baseUrl) {
    ...
    return function (params) {
        return new promise(function (resolve, reject) {
            ...
            if (qs) {
                uri = uri.replace(/:(\w+)/g, function (all, name) {
            ...
            var requestOptions = params;

            if (!params.uri) {
                requestOptions.uri = uri.indexOf('http://') > -1 ? uri : baseUrl + uri;
                requestOptions.method = route.method;

                if (route.json) {
                    requestOptions.json = route.json;
                }
            }
